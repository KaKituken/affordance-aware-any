...
config = OmegaConf.load('../config/compress512.yaml')
model = instantiate_from_config(config.model)

print('loading checkpoint...')
model.load_state_dict(torch.load('../../ckpt/affordance.ckpt')["state_dict"], strict=False)
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Accordion("Advanced options", open=False):
                ...
                seed = gr.Slider(label="Seed", minimum=-1, maximum=5000, step=1, randomize=True)
